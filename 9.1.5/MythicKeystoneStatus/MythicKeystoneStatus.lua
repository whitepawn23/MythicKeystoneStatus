local _, MythicKeystoneStatus = ...
MythicKeystoneStatus = LibStub("AceAddon-3.0"):NewAddon("MythicKeystoneStatus", "AceConsole-3.0", "AceEvent-3.0", "AceTimer-3.0","AceHook-3.0" );
local L = LibStub("AceLocale-3.0"):GetLocale("MythicKeystoneStatus")
_G.MythicKeystoneStatus = MythicKeystoneStatus
...
local function GetCharacterInfo()
	if (UnitLevel("player") < levelCap) then return end
...
function MythicKeystoneStatus:PLAYER_ENTERING_WORLD(event, payload)	
	DelayedUpdate()
	-- MythicKeystoneStatus:Print(string.format("New weekly record for %s [+%d] %s",C_ChallengeMode.GetMapUIInfo(payload.mapChallengeModeID), payload.level, SecondsToTime(payload.completionMilliseconds/1000)))
end

function MythicKeystoneStatus:CHALLENGE_MODE_COMPLETED(event, payload)
	DelayedUpdate()
	--MythicKeystoneStatus:Print(string.format("New season record for %s [+%d] %s",C_ChallengeMode.GetMapUIInfo(payload.mapChallengeModeID), payload.level, SecondsToTime(payload.completionMilliseconds/1000)))
end
...
local function GetCharacters(realm)
	...
	for key,value in pairs(characters) do
		if ((not realm) or (realm == characters[i].realm) and value.level == levelCap) then
...
function MythicKeystoneStatus:ShowSubTooltip(cell, info)	
	...
	for i = 1, #dungeons do
		...
		if status then
			if status.time then
				subTooltip:SetCell(line, 2, SecondsToTime(status.time), nil, "LEFT")
			else
